...
/****** Object:  View [dbo].[VISTA_PRESTACIONES_POR_FACTURAR]    Script Date: 09/11/2012 09:41:10 ******/
SET ANSI_NULLS ON
GO
...
WHERE     (dbo.PRESTACION.ACTIVO = 1) AND (dbo.CLIENTE.ACTIVO = 1) AND (NOT EXISTS
	(SELECT     1 AS Expr1
		...
		INNER JOIN dbo.FACTURA ON dbo.FACTURA_DETALLE.ID_FACTURA = dbo.FACTURA.ID
		INNER JOIN dbo.FACTURACION ON dbo.FACTURA.ID_FACTURACION = dbo.FACTURACION.ID
		
		WHERE dbo.FACTURA.ACTIVO = 1 AND 
		dbo.FACTURACION.ACTIVO = 1 AND
		(dbo.FACTURA_DETALLE.ACTIVO = 1) AND (dbo.FACTURA_DETALLE.NUMERO_FICHA = dbo.PRESTACION.ID)))
