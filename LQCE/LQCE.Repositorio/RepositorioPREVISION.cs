	...
	public partial class RepositorioPREVISION
	{
		...
		public PREVISION GetByIdWithReferences(int id)
		{
			...
			try
			{
							...
							return _context.PREVISION.Include("CARGA_PRESTACIONES_HUMANAS_DETALLE").Include("CARGA_PRESTACIONES_VETERINARIAS_DETALLE").Include("PRESTACION").FirstOrDefault(i => i.ID == id && i.ACTIVO );
		...
		public IQueryable<PREVISION> GetAllWithReferences()
		{
			...
			try
			{
								...
								var q = from i in _context.PREVISION.Include("CARGA_PRESTACIONES_HUMANAS_DETALLE").Include("CARGA_PRESTACIONES_VETERINARIAS_DETALLE").Include("PRESTACION") where i.ACTIVO  select i;
							return q;
			}
			catch (Exception ex)
            {
